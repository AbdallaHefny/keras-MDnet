...
result = np.zeros((nFrames, 4))
reg_result = np.zeros((nFrames, 4))
result[0] = reg_result[0] = targetLoc

net, conv_net, fc_net, net_body, bbox_reg_net= initialize_model()
...
bbox_reg = BBoxRegressor(bbox_reg_net)
...
#fine tune FC layers
multipliers = {'fc6': 10}
opt = LearningRateMultiplier(optimizers.SGD, lr_multipliers=multipliers, lr=0.0001, decay=0.0005, momentum=0.9)
...
ishow = ax.imshow(img)
ax.add_patch(patches.Rectangle((targetLoc[0], targetLoc[1]), targetLoc[2] , targetLoc[3],
                            fill=False, color='red'))
ax.add_patch(patches.Rectangle((gt[0][0], gt[0][1]), gt[0][2] , gt[0][3],
...
multipliers = {'fc6': 10}
opt = LearningRateMultiplier(optimizers.SGD, lr_multipliers=multipliers, lr=0.0003, decay=0.0005, momentum=0.9)
fc_net.compile(loss='categorical_crossentropy', optimizer=opt)
...
for f in range(1,nFrames):
    ...
    if target_score < success_thr:
        ...
        targetLoc = result[f-1] ## only torch doing this
    else:
       trans_f = 0.6
    ...
    ax = plt.subplot(1,1,1)
    ishow=ax.imshow(img) 
    ax.add_patch(patches.Rectangle((result[f][0], result[f][1]), result[f][2] , result[f][3],
                               fill=False, color='red'))
    ax.add_patch(patches.Rectangle((reg_targetLoc[0], reg_targetLoc[1]), reg_targetLoc[2] , reg_targetLoc[3],
                               fill=False, color='green', lw = 2))
    ax.add_patch(patches.Rectangle((gt[f][0], gt[f][1]), gt[f][2] , gt[f][3],
                               fill=False, color='blue'))
    plt.pause(.1)
#     fig.savefig('%04d.jpg'%(f))
    plt.show();   