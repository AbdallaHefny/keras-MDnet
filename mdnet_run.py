...
rect_regressor = patches.Rectangle((targetLoc[0], targetLoc[1]), targetLoc[2] , targetLoc[3],
                            fill=False, color='green')
ax.add_patch(rect_gt)
ax.add_patch(rect_target)
ax.add_patch(rect_regressor)
plt.pause(0.1)
plt.draw()
...
for f in range(1,nFrames):
    ...
    rect_target.set_xy((result[f][0], result[f][1]))
    rect_target.set_width(result[f][2])
    rect_target.set_height(result[f][3])
    
    rect_regressor.set_xy((reg_targetLoc[0], reg_targetLoc[1]))
    rect_regressor.set_width(reg_targetLoc[2])
    rect_regressor.set_height(reg_targetLoc[3])    
